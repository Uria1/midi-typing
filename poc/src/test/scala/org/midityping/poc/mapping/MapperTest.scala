...
class MapperTest extends SpecificationWithJUnit with TestSupport {
  ...
  val mapC4andF4 = Mapping((StrikeD(Seq(EventD("C4"), EventD("F4")), Mode.default), ActionD("X")))
  ...
  "Mapper" >> {
    ...
    "map strike in different modes" in {
      val mapper = new Mapper
      mapper.appendMapping(CustomMappingLoader.load("/map-c4.mdt"))
      mapper.appendMapping(CustomMappingLoader.load("/map-c4-numbers-mode.mdt"))
      mapper.getActionDescriptor(Strike(anEvent(Note.C4)), "default") === Some(ActionD("C"))
      mapper.getActionDescriptor(Strike(anEvent(Note.C4)), "numbers") === Some(ActionD("1"))
    }
  }
}
